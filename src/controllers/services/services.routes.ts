...
router.route('/:id').patch(authMiddleware.isAuthorized, servicesController.Editservices);
...
router.route('/:id/section/:id').patch(upload.single('sectionImage'),servicesController.Editsection);
router.route('/:id/section').post(authMiddleware.isAuthorized, upload.single('sectionImage'), servicesController.AddServiceSection);
